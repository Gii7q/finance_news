...
def save_to_db(articles):
    ...
    try:
        ...
        for art in articles:
            try:
                c.execute("INSERT OR IGNORE INTO news (title, summary, published, link, source, category, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                          (art.get("title"), art.get("summary"), art.get("published"), art.get("link"), art.get("source"), art.get("category", "综合资讯"), datetime.now().isoformat()))
                if c.rowcount > 0:
                    new_count += 1
            except Exception as e:
                logger.error(f"入库出错: {e}")
        conn.commit()
        conn.close()
        logger.info(f"本地数据库新增 {new_count} 条新闻")
        return new_count
    except Exception as e:
...
def generate_daily_summary():
    try:
        ...
        logger.info(f"📅 查询的昨天日期: {yesterday}")
        c.execute("SELECT title, link, published, summary FROM news WHERE substr(created_at, 1, 10) = ? ORDER BY created_at DESC", (yesterday,))
        rows = c.fetchall()
        conn.close()
...
def handler(event, context):
    ...
    由定时触发器调用
    """
    logger.info("🚀 云函数定时任务触发")
    try:
        main()
        return {
            "statusCode": 200,
            "body": json.dumps({"message": "任务执行成功"})
        }
    except Exception as e:
        logger.error(f"❌ 任务执行失败: {str(e)}")
        return {
            "statusCode": 500,
            "body": json.dumps({"message": f"任务执行失败: {str(e)}"})
        }
...
def main():
    ...
    clean_old_news()  # 清理30天前的旧新闻
    
    articles = fetch_news()
    if articles:
        # 1. 先保存到本地数据库
        save_to_db(articles)
        # 2. 再推送到 PythonAnywhere
